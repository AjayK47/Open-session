...
import { Button, Input, Label, Textarea } from "@opensession/ui";
...
export function OrganizationOnboardingPage() {
  ...
  return (
    <div className="grid min-h-screen bg-background lg:grid-cols-[.82fr_1.18fr]">
      <aside className="relative hidden overflow-hidden border-r border-border bg-[#20231e] p-12 text-white lg:flex lg:flex-col">
        ...
        <div className="relative mt-auto max-w-md">
          ...
          <h1 className="mt-4 font-serif text-5xl leading-[1.02] tracking-tight">Give every event a place to belong.</h1>
          <p className="mt-5 text-[15px] leading-7 text-white/60">Your organization holds the team, brand, and events for this deployment. You can change these details whenever you need.</p>
      ...
      <main className="flex items-center justify-center px-6 py-12 sm:px-10">
        <form onSubmit={form.handleSubmit(submit)} className="w-full max-w-2xl">
          <div className="mb-9"><span className="flex size-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><Building2 className="size-5" /></span><h2 className="mt-5 text-3xl font-semibold tracking-tight">Create your organization</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">This is the home for your team and every event you run.</p></div>
          ...
          <div className="grid gap-6 sm:grid-cols-[9rem_1fr]">
            ...
            <div className="space-y-5">
              ...
              <Field label="Default timezone" error={form.formState.errors.default_timezone?.message}><Input {...form.register("default_timezone")} /></Field>
              <Field label="About your organization" error={form.formState.errors.description?.message}><Textarea rows={4} placeholder="A short description for your team workspace." {...form.register("description")} /></Field>
            </div>
          </div>

          <div className="mt-8 flex items-center justify-end border-t border-border pt-6"><Button size="lg" type="submit" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting ? "Creating…" : <>Create workspace <ArrowRight /></>}</Button></div>
        </form>
      </main>
    </div>
  );
